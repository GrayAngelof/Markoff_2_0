...
class EventBus:
    ...
    def __init__(self, debug: bool = False) -> None:
        ...
        self._callbacks: Dict[int, Union[Callable, weakref.ReferenceType]] = {}
        self._next_id = 0
    ...
    def get_subscriber_count(self, event_type: Optional[Type[EventData]] = None) -> Union[int, Dict[str, int]]:
        """
        Возвращает количество подписчиков. ТОЛЬКО ДЛЯ ТЕСТОВ И ОТЛАДКИ.
    ...
    def _get_callback_info(self, callback: Callable) -> tuple[str, str]:
        """Возвращает информацию о callback для логирования: (тип_ссылки, описание)."""
        if inspect.ismethod(callback):
