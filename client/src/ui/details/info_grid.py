...
TECHNICAL DEBT:
    - Добавить поддержку ссылок (URL план этажа) — кликабельные значения
    - Добавить поддержку многострочных значений (сейчас QLabel с WordWrap)
    - Добавить форматирование специальных типов (площадь, даты, ID владельца)
"""
...
class InfoGrid(QWidget):
    ...
    TECHNICAL DEBT:
        - URL план этажа → сделать кликабельным (QLabel → QLabel с открытием браузера)
        - Площадь → добавить форматирование (уже есть, но может потребоваться точность)
        - Даты → добавить единый формат (используется format_timestamp из shared.time)
        - ID владельца → маппить на имя через репозиторий
    """
