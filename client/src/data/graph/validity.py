...
from typing import Dict, Set, List, Optional, Callable, Final, Iterable, TypedDict
...
class ValidityIndex:
    ...
    def _invalidate_node(self, node_type: NodeType, entity_id: int) -> bool:
        """
        Внутренний метод: инвалидирует один узел.
    ...
    def invalidate_branch(self, node_type: NodeType, entity_id: int,
        ...
        Использует очередь/стек для обхода дерева, что предотвращает
        ...
        with self._lock:
            ...
            count = 0
            # Используем очередь для BFS-обхода (можно заменить на стек для DFS)
            queue = deque()
    ...
    def clear(self) -> None:
        """Полностью очищает индекс валидности (все сущности становятся невалидными)."""
        with self._lock:
