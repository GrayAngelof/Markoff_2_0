...
class LoadStateIndex:
    ...
    def __init__(self) -> None:
        ...
        log.system("LoadStateIndex инициализация")
        self._lock = RLock()

        # Состояние для каждого типа узла: {id: LoadState}
        # Только физическая иерархия (контрагенты не входят в дерево)
    ...
    def is_loaded(self, node_type: NodeType, node_id: int) -> bool:
        """Проверяет, загружены ли дети узла."""
        with self._lock:
    ...
    def mark_loading(self, node_type: NodeType, node_id: int) -> bool:
        ...
        Returns:
            True если загрузка начата, False если уже загружается или загружена
        """
        with self._lock:
    ...
    def mark_loaded(self, node_type: NodeType, node_id: int) -> None:
        """Помечает успешное завершение загрузки."""
        with self._lock:
    ...
    def mark_failed(self, node_type: NodeType, node_id: int) -> None:
        """Помечает ошибку загрузки (сброс в NOT_LOADED)."""
        with self._lock:
