...
Композиция:
- NodeLoader: чистая логика загрузки (loader.py)
- EventHandler: обработка событий (events.py)
- LoaderUtils: вспомогательные функции (utils.py)

Использование:
    loader = DataLoader(event_bus, api_client, graph)
    loader.load_complexes()  # прямая загрузка
    # или через события: event_bus.emit('ui.node_expanded', {...})
"""
