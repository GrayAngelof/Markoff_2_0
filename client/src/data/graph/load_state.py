...
class LoadStateIndex:
    ...
    Поддерживает потокобезопасность через RLock.
    Предотвращает гонки при параллельных запросах.
    
    Состояния:
    - NOT_LOADED: дети не загружались (первый запрос)
    - LOADING: загрузка в процессе (ждем завершения)
    - LOADED: дети загружены (можно использовать)
    """
    ...
    def get_state(self, node_type: NodeType, node_id: int) -> LoadState:
        ...
        Returns:
            LoadState: Текущее состояние (по умолчанию NOT_LOADED)
        """
        with self._lock:
    ...
    def is_loading(self, node_type: NodeType, node_id: int) -> bool:
        ...
        Returns:
            bool: True если загрузка в процессе
        """
        with self._lock:
    ...
    def mark_loading(self, node_type: NodeType, node_id: int) -> bool:
        ...
        Returns:
            bool: True если загрузка начата, False если уже загружается или загружено
        """
        with self._lock:
    ...
    def get_stats(self) -> Dict[str, int]:
        ...
        Returns:
            Dict[str, int]: Статистика по состояниям
        """
        with self._lock:
            stats = {
                'not_loaded': 0,
